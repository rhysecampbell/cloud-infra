...
open (DATA, "$path/carved/$filename");
open (XML, ">$path/xml-new/$filename.xml");
...
foreach $cloudcode (@cloudCodes) {
	...
	# only carve up the skyCondition if it's XXXnnn
	if ($cloudcode =~ m/^([A-Z]{3})(\d{3})$/) {
		$cloudcode1=$1;
		$cloudcode2=$2;
		print XML "          <vai3:value no=\"$cloudcodeCount\" code=\"${typeCode}_skyCondition$cloudcode1\">$cloudcode2</vai3:value>\n";
		}
